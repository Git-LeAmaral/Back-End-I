...
public class GamesController implements GamesApi {
    ...
    @Override
    public ResponseEntity<Page<GameSumaryResponse>> buscarGames(Pageable pageable) {
        Page<Game> gamePage = gameService.buscarGames(pageable);

        Page<GameSumaryResponse> response = gamePage
                .map(game -> new GameSumaryResponse(game.getId(), game.getNome()));
